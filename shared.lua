...
function shared_init()
  ...
  -- Custom procedural cursor with animation
  love.mouse.setVisible(false)
  cursor = {x = 0, y = 0, time = 0, pulse = 0}
end
...
local x, y = 0, 0
  ...
  -- Draw animated procedural crosshair cursor on top of everything
  local mx, my = love.mouse.getPosition()
  cursor.x, cursor.y = mx, my
  cursor.time = cursor.time + love.timer.getDelta()
  
  -- Subtle breathing effect
  local breathe = math.sin(cursor.time * 2) * 0.08 + 1
  -- Spin when shooting
  local shooting = input and input.m1 and input.m1.down
  cursor.spin = cursor.spin or 0
  if shooting then
    cursor.spin = cursor.spin + love.timer.getDelta() * 12
  else
    cursor.spin = cursor.spin * 0.9 -- Decay
  end
  
  local c = fg[0]
  love.graphics.setColor(c.r, c.g, c.b, c.a)
  love.graphics.setLineStyle("rough")
  love.graphics.setLineWidth(6) -- Thicker for pill look
  
  local cs = 28 * breathe
  local arm_len = 18 * breathe
  local corner_radius = 4
  
  -- Draw pill-shaped brackets (rounded corners like characters)
  love.graphics.push()
  love.graphics.translate(mx, my)
  love.graphics.rotate(cursor.spin)
  
  -- Top-left pill bracket
  love.graphics.line(-cs, -cs + corner_radius, -cs, -cs + arm_len)
  love.graphics.line(-cs + corner_radius, -cs, -cs + arm_len, -cs)
  
  -- Top-right pill bracket
  love.graphics.line(cs, -cs + corner_radius, cs, -cs + arm_len)
  love.graphics.line(cs - corner_radius, -cs, cs - arm_len, -cs)
  
  -- Bottom-left pill bracket
  love.graphics.line(-cs, cs - corner_radius, -cs, cs - arm_len)
  love.graphics.line(-cs + corner_radius, cs, -cs + arm_len, cs)
  
  -- Bottom-right pill bracket
  love.graphics.line(cs, cs - corner_radius, cs, cs - arm_len)
  love.graphics.line(cs - corner_radius, cs, cs - arm_len, cs)
  
  love.graphics.pop()
  
  -- Center pill dot (breathing)
  love.graphics.setLineWidth(4)
  local center_size = 5 * breathe
  love.graphics.rectangle("fill", mx - center_size/2, my - center_size/2, center_size, center_size, 2, 2)
