...
local TYPES = {
  chaser  = { hp = 1, speed = 42, r = 2.5, color = gfx.COLOR_PINK,        score = 10 },
  fast    = { hp = 1, speed = 80, r = 1.5, color = gfx.COLOR_YELLOW,      score = 20 },
  shooter = { hp = 2, speed = 28, r = 3,   color = gfx.COLOR_BLUE,        score = 25 },
  tank    = { hp = 8, speed = 16, r = 5,   color = gfx.COLOR_DARK_PURPLE, score = 50 },
}

local function spawn_pos()
  local edge = math.random(1, 4)
  if edge == 1 then return -8, math.random(0, usagi.GAME_H) end
  if edge == 2 then return usagi.GAME_W + 8, math.random(0, usagi.GAME_H) end
  if edge == 3 then return math.random(0, usagi.GAME_W), -8 end
...
function Enemies.update(dt, State)
  ...
  for i = #Enemies.list, 1, -1 do
    ...
    if e.knock > 0 then
      ...
      if e.knock < 0 then e.knock = 0 end
      e.x = e.x + e.kx * dt
      e.y = e.y + e.ky * dt
    end
...
function Enemies.kill(e, State)
  for i = #Enemies.list, 1, -1 do
    if Enemies.list[i] == e then table.remove(Enemies.list, i) end
  end
