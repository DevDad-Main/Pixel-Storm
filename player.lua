...
function Player.update(p, dt, State)
  ...
  if (dx ~= 0 or dy ~= 0) and p.trail_t <= 0 then
    p.trail_t = 0.04
    Particles.emit({x=p.x, y=p.y, count=1, speed=4, life=0.35, color=gfx.COLOR_GREEN, size=1, drag=2})
  end
...
function Player.hit(p, dmg, State)
  if p.invuln_t > 0 or not p.alive then return end
  p.hp = p.hp - dmg
  p.invuln_t = 0.8
...
function Player.draw(p)
  if not p.alive then return end
  if p.invuln_t > 0 and util.flash(usagi.elapsed, 16) then return end
